...
cd $loc rm *.deb >/dev/null 2>&1
sleep 4.0
echo -e "/e[34mREMOVED METASPLOIT SUCCESSFULLY.....[/e[92m✓/e[34m]/e[92m"
sleep 4.0
else
echo
fi
...
cd $loc/m-wiz-master/m-wiz-master/core/rb/arm
...
tar -xf $ver.tar.gz >/dev/null 2>&1
mv $loc/metasploit-framework-$ver $loc/metasploit-framework >/dev/null 2>&1
cd $loc/m-wiz-master/m-wiz-master/core/rb/aarch64
cp ruby.deb $loc >/dev/null 2>&1
cd $loc
apt install -y ./ruby.deb >/dev/null 2>&1
apt-mark hold ruby >/dev/null 2>&1
